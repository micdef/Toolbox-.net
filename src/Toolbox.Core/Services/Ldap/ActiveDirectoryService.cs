...
{
    /// <summary>
    /// The Active Directory configuration options.
    /// </summary>
    ...
    /// <summary>
    /// The LDAP connection to Active Directory.
    /// </summary>
    private LdapConnection? _connection;

    /// <summary>
    /// Indicates whether the service is currently connected to Active Directory.
    /// </summary>
    ...
    /// <summary>
    /// Searches for users matching the specified filter with pagination support.
    /// </summary>
    ...
    /// <summary>
    /// Builds an LDAP filter string from user search criteria.
    /// </summary>
    ...
    /// <summary>
    /// Escapes special characters in an LDAP filter value while preserving wildcard characters.
    /// </summary>
    /// <param name="value">The value to escape.</param>
    /// <returns>The escaped value safe for use in LDAP filters, with asterisks preserved as wildcards.</returns>
    ...
    /// <summary>
    /// Searches for groups matching the specified filter with pagination support.
    /// </summary>
    /// <param name="filter">The LDAP filter to apply.</param>
    /// <param name="page">The page number (1-based).</param>
    /// <param name="pageSize">The number of items per page.</param>
    ...
    /// <summary>
    /// Builds an LDAP filter string from group search criteria.
    /// </summary>
    /// <param name="criteria">The search criteria to convert to an LDAP filter.</param>
    /// <returns>An LDAP filter string combining all specified criteria with AND logic.</returns>
    ...
    /// <summary>
    /// Maps an LDAP search result entry to an LdapGroup object.
    /// </summary>
    /// <param name="entry">The LDAP search result entry to map.</param>
    /// <returns>An LdapGroup object populated from the entry attributes.</returns>
    ...
    /// <summary>
    /// Parses the Active Directory groupType attribute value.
    /// </summary>
    /// <param name="groupTypeValue">The groupType attribute value as a string.</param>
    /// <returns>A tuple containing the group type, scope, and security flag.</returns>
    ...
    /// <summary>
    /// Retrieves a multi-valued attribute from an LDAP entry.
    /// </summary>
    /// <param name="entry">The LDAP search result entry.</param>
    /// <param name="attributeName">The name of the attribute to retrieve.</param>
    /// <returns>A list of values, or an empty list if the attribute is not present.</returns>
    ...
    /// <summary>
    /// Gets the list of LDAP attributes to retrieve for group queries.
    /// </summary>
    ...
    /// <summary>
    /// Searches for computers matching the specified filter with pagination support.
    /// </summary>
    /// <param name="filter">The LDAP filter to apply.</param>
    /// <param name="page">The page number (1-based).</param>
    /// <param name="pageSize">The number of items per page.</param>
    ...
    /// <summary>
    /// Builds an LDAP filter string from computer search criteria.
    /// </summary>
    /// <param name="criteria">The search criteria to convert to an LDAP filter.</param>
    /// <returns>An LDAP filter string combining all specified criteria with AND logic.</returns>
    ...
    /// <summary>
    /// Maps an LDAP search result entry to an LdapComputer object.
    /// </summary>
    /// <param name="entry">The LDAP search result entry to map.</param>
    /// <returns>An LdapComputer object populated from the entry attributes.</returns>
    ...
    /// <summary>
    /// Gets the list of LDAP attributes to retrieve for computer queries.
    /// </summary>
    /// <returns>An array of attribute names.</returns>
    ...
    /// <summary>
    /// Authenticates using anonymous bind (no credentials).
    /// </summary>
    ...
    /// <summary>
    /// Authenticates using Kerberos protocol.
    /// </summary>
    /// <param name="options">The authentication options containing optional credentials.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The authentication result indicating success or failure.</returns>
    ...
    /// <summary>
    /// Authenticates using NTLM protocol.
    /// </summary>
    /// <param name="options">The authentication options containing optional credentials.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The authentication result indicating success or failure.</returns>
    ...
    /// <summary>
    /// Authenticates using Negotiate protocol (SPNEGO - Kerberos with NTLM fallback).
    /// </summary>
    /// <param name="options">The authentication options containing optional credentials.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The authentication result indicating success or failure.</returns>
    ...
    /// <summary>
    /// Authenticates using integrated Windows authentication (current user context).
    /// </summary>
    /// <param name="options">The authentication options.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The authentication result indicating success or failure.</returns>
    ...
    /// <summary>
    /// Attempts certificate-based authentication.
    /// </summary>
    /// <param name="options">The authentication options containing certificate information.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The authentication result (currently returns failure as certificate auth requires platform-specific configuration).</returns>
    ...
    /// <returns>The extracted username, defaulting to the certificate thumbprint if CN is not found.</returns>
    ...
    /// <summary>
    /// Enriches an authentication result with additional user information such as groups and claims.
    /// </summary>
    /// <param name="result">The initial authentication result to enrich.</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            await EnsureConnectedAsync(cancellationToken);
            var dn = await ResolveDistinguishedNameAsync(options, cancellationToken);
            ...
            var currentUac = await GetUserAccountControlAsync(dn, cancellationToken);
            if (!currentUac.HasValue)
            {
                return LdapManagementResult.Failure(
                    LdapManagementOperation.EnableAccount,
                    "Could not read userAccountControl attribute.",
                    targetDn: dn);
            }

            // Remove ACCOUNTDISABLE flag (0x2)
            var newUac = currentUac.Value & ~0x2;
            ...
            await Task.Run(() => _connection!.SendRequest(modifyRequest), cancellationToken);

            _logger.LogInformation("Enabled account: {DistinguishedName}", dn);
        ...
        {
            _logger.LogError(ex, "Failed to enable account");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Disabled account: {DistinguishedName}", dn);
        ...
        {
            _logger.LogError(ex, "Failed to disable account");
    ...
    {
        return UnlockAccountAsync(options).GetAwaiter().GetResult();
    }

    /// <inheritdoc />
    public async Task<LdapManagementResult> UnlockAccountAsync(
        LdapAccountOptions options,
        CancellationToken cancellationToken = default)
    {
        ThrowIfDisposed();
        ...
        try
        {
            ...
            // Set lockoutTime to 0 to unlock
            var modifyRequest = new ModifyRequest(
                dn,
                DirectoryAttributeOperation.Replace,
                "lockoutTime",
            ...
            _logger.LogInformation("Unlocked account: {DistinguishedName}", dn);
            ToolboxMeter.RecordLdapManagement(ServiceName, "UnlockAccount", true);
        ...
        {
            _logger.LogError(ex, "Failed to unlock account");
            ToolboxMeter.RecordLdapManagement(ServiceName, "UnlockAccount", false);
            return LdapManagementResult.Failure(
                LdapManagementOperation.UnlockAccount,
                ex.Message,
                ex.ErrorCode);
    ...
    {
        return SetAccountExpirationAsync(options).GetAwaiter().GetResult();
    }

    /// <inheritdoc />
    public async Task<LdapManagementResult> SetAccountExpirationAsync(
        LdapAccountOptions options,
        CancellationToken cancellationToken = default)
    {
        ThrowIfDisposed();
        ...
        try
        {
            ...
            DirectoryAttributeModification mod;

            if (options.ClearExpiration || options.ExpirationDate == null)
            {
                // Set to 0 (never expires) or remove the attribute
                mod = new DirectoryAttributeModification
                {
                    Name = "accountExpires",
            ...
            else
            {
                ...
                var fileTime = options.ExpirationDate.Value.ToFileTime();
                mod = new DirectoryAttributeModification
                {
                    Name = "accountExpires",
                    Operation = DirectoryAttributeOperation.Replace
                };
                mod.Add(fileTime.ToString());
            }

            var modifyRequest = new ModifyRequest(dn, mod);
            await Task.Run(() => _connection!.SendRequest(modifyRequest), cancellationToken);

            var message = options.ClearExpiration
                ? "Account expiration cleared."
                : $"Account expiration set to {options.ExpirationDate:O}.";

            _logger.LogInformation("Set account expiration for {DistinguishedName}: {Message}", dn, message);
            ToolboxMeter.RecordLdapManagement(ServiceName, "SetAccountExpiration", true);

            return LdapManagementResult.Success(
                options.ClearExpiration ? LdapManagementOperation.ClearAccountExpiration : LdapManagementOperation.SetAccountExpiration,
                dn,
                message);
        }
        catch (LdapException ex)
        {
            _logger.LogError(ex, "Failed to set account expiration");
            ToolboxMeter.RecordLdapManagement(ServiceName, "SetAccountExpiration", false);
            return LdapManagementResult.Failure(
                LdapManagementOperation.SetAccountExpiration,
                ex.Message,
                ex.ErrorCode);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return LdapManagementResult.Success(
                LdapManagementOperation.AddToGroup,
                groupDn,
                $"Member {memberDn} added to group.");
        }
        catch (LdapException ex) when (ex.ErrorCode == 68) // Entry already exists
        {
            _logger.LogDebug("Member already exists in group");
            return LdapManagementResult.Success(
                LdapManagementOperation.AddToGroup,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return LdapManagementResult.Success(
                LdapManagementOperation.RemoveFromGroup,
                groupDn,
                $"Member {memberDn} removed from group.");
        }
        catch (LdapException ex) when (ex.ErrorCode == 32) // No such object (member not in group)
        {
            _logger.LogDebug("Member not found in group");
            return LdapManagementResult.Success(
                LdapManagementOperation.RemoveFromGroup,
    ...
    {
        return MoveObjectAsync(options).GetAwaiter().GetResult();
    }

    /// <inheritdoc />
    public async Task<LdapManagementResult> MoveObjectAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // Extract CN from source DN
            var sourceDn = options.SourceDistinguishedName!;
            var cnStart = sourceDn.IndexOf("CN=", StringComparison.OrdinalIgnoreCase);
            var cnEnd = sourceDn.IndexOf(',', cnStart);
            var currentCn = sourceDn.Substring(cnStart, cnEnd - cnStart);

            var newRdn = string.IsNullOrEmpty(options.NewCommonName)
                ? currentCn
                : $"CN={EscapeLdapDnValue(options.NewCommonName)}";

            var modifyDnRequest = new ModifyDNRequest(
                sourceDn,
                options.TargetOrganizationalUnit,
                newRdn)
            {
                DeleteOldRdn = options.DeleteOldRdn
            };

            await Task.Run(() => _connection!.SendRequest(modifyDnRequest), cancellationToken);

            _logger.LogInformation(
                "Moved object from {SourceDn} to {TargetOu}",
                sourceDn,
                options.TargetOrganizationalUnit);
            ToolboxMeter.RecordLdapManagement(ServiceName, "MoveObject", true);

            return LdapManagementResult.Success(
                LdapManagementOperation.MoveObject,
                options.TargetDistinguishedName,
                $"Object moved from {sourceDn} to {options.TargetDistinguishedName}.");
        }
        catch (LdapException ex)
        {
            _logger.LogError(ex, "Failed to move object");
            ToolboxMeter.RecordLdapManagement(ServiceName, "MoveObject", false);
            return LdapManagementResult.Failure(
                LdapManagementOperation.MoveObject,
                ex.Message,
                ex.ErrorCode,
                options.SourceDistinguishedName);
        }
    }
    ...
    {
        return RenameObjectAsync(distinguishedName, newCommonName).GetAwaiter().GetResult();
    }

    /// <inheritdoc />
    public async Task<LdapManagementResult> RenameObjectAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ArgumentException.ThrowIfNullOrEmpty(newCommonName);
        cancellationToken.ThrowIfCancellationRequested();
        ...
        try
        {
            ...
            var newRdn = $"CN={EscapeLdapDnValue(newCommonName)}";

            var modifyDnRequest = new ModifyDNRequest(
                distinguishedName,
                null, // Stay in same container
                newRdn)
            {
                DeleteOldRdn = true
            };
            ...
            _logger.LogInformation("Renamed object {Dn} to {NewCn}", distinguishedName, newCommonName);
            ToolboxMeter.RecordLdapManagement(ServiceName, "RenameObject", true);

            return LdapManagementResult.Success(
                LdapManagementOperation.RenameObject,
                distinguishedName,
                $"Object renamed to {newCommonName}.");
        }
        catch (LdapException ex)
        {
            _logger.LogError(ex, "Failed to rename object");
            ToolboxMeter.RecordLdapManagement(ServiceName, "RenameObject", false);
            return LdapManagementResult.Failure(
                LdapManagementOperation.RenameObject,
                ex.Message,
                ex.ErrorCode,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // AD requires special encoding for password change
            var oldPasswordBytes = System.Text.Encoding.Unicode.GetBytes($"\"{options.CurrentPassword}\"");
            var newPasswordBytes = System.Text.Encoding.Unicode.GetBytes($"\"{options.NewPassword}\"");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            // AD requires special encoding for password reset
            var newPasswordBytes = System.Text.Encoding.Unicode.GetBytes($"\"{options.NewPassword}\"");
            ...
            if (options.MustChangePasswordAtNextLogon)
            {
                await ForcePasswordChangeAtNextLogonInternalAsync(dn, cancellationToken);
            }

            if (options.UnlockAccount)
            {
                var unlockRequest = new ModifyRequest(
                    dn,
                    DirectoryAttributeOperation.Replace,
                    "lockoutTime",
                    "0");
                await Task.Run(() => _connection!.SendRequest(unlockRequest), cancellationToken);
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Set force password change for user: {DistinguishedName}", dn);
        ...
        {
            _logger.LogError(ex, "Failed to set force password change");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var newUac = neverExpires
                ? currentUac.Value | 0x10000
                : currentUac.Value & ~0x10000;
            ...
            var operation = neverExpires
                ? LdapManagementOperation.SetPasswordNeverExpires
                : LdapManagementOperation.ClearPasswordNeverExpires;

            _logger.LogInformation("Set password never expires to {Value} for user: {Dn}", neverExpires, dn);
            ToolboxMeter.RecordLdapManagement(ServiceName, "SetPasswordNeverExpires", true);

            return LdapManagementResult.Success(
                operation,
                dn,
                neverExpires ? "Password set to never expire." : "Password expiration enabled.");
        }
        catch (LdapException ex)
        {
            _logger.LogError(ex, "Failed to set password never expires");
    ...
    public IReadOnlyList<LdapManagementOperation> GetSupportedManagementOperations()
    {
        ...
        [
            ...
            LdapManagementOperation.ClearPasswordNeverExpires,
            LdapManagementOperation.SetAccountExpiration,
            LdapManagementOperation.ClearAccountExpiration,
            LdapManagementOperation.ModifyAttribute
        ];
    }
    ...
    /// <summary>
    /// Resolves the distinguished name of an account from the provided options.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var filter = options.ObjectType == LdapObjectType.Computer
            ? $"(&(objectClass=computer)(sAMAccountName={EscapeLdapFilter(options.Username)}$))"
            : $"(&(objectClass=user)(sAMAccountName={EscapeLdapFilter(options.Username)}))";
    ...
    /// <summary>
    /// Resolves the distinguished name of a user from password options.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var filter = $"(&(objectClass=user)(sAMAccountName={EscapeLdapFilter(options.Username)}))";
    ...
    /// <summary>
    /// Resolves the distinguished name of a group from the provided options.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var filter = $"(&(objectClass=group)(|(cn={EscapeLdapFilter(options.GroupName)})(sAMAccountName={EscapeLdapFilter(options.GroupName)})))";
    ...
    /// <summary>
    /// Resolves the distinguished name of a member (user, computer, or group) from the provided options.
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        var filter = $"(&(|(objectClass=user)(objectClass=computer)(objectClass=group))(sAMAccountName={EscapeLdapFilter(options.MemberUsername)}))";
    ...
    /// <summary>
    /// Retrieves the userAccountControl attribute value for a user.
    /// </summary>
    /// <param name="distinguishedName">The distinguished name of the user.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The userAccountControl value, or null if not found.</returns>
    private async Task<int?> GetUserAccountControlAsync(
        string distinguishedName,
        CancellationToken cancellationToken)
    {
        var searchRequest = new SearchRequest(
        ...
        var uacValue = GetAttributeValue(response.Entries[0], "userAccountControl");
        return int.TryParse(uacValue, out var uac) ? uac : null;
    }

    /// <summary>
    /// Forces a user to change their password at next logon by setting pwdLastSet to 0.
    /// </summary>
    /// <param name="distinguishedName">The distinguished name of the user.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    ...
    /// <summary>
    /// Ensures that a connection to Active Directory is established.
    /// </summary>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>A task representing the asynchronous operation.</returns>
    ...
    /// using the configured options (server, port, SSL, credentials).
    /// </remarks>
    ...
    /// <summary>
    /// Maps a search result entry to an LdapUser object.
    /// </summary>
    /// <param name="entry">The search result entry from Active Directory.</param>
    /// <returns>A populated LdapUser object.</returns>
    /// <remarks>
    /// Maps standard Active Directory attributes to LdapUser properties,
    /// including account status, group membership, and custom attributes.
    /// </remarks>
    ...
    /// <summary>
    /// Gets the string value of an attribute from a search result entry.
    /// </summary>
    /// <param name="entry">The search result entry.</param>
    /// <param name="attributeName">The name of the attribute to retrieve.</param>
    ...
    /// <summary>
    /// Determines whether an account is disabled based on userAccountControl flags.
    /// </summary>
    /// <param name="entry">The search result entry containing userAccountControl.</param>
    /// <returns>True if the account is disabled; otherwise, false.</returns>
    /// <remarks>
    /// Checks the ACCOUNTDISABLE flag (0x2) in the userAccountControl attribute.
    /// </remarks>
    ...
    /// <summary>
    /// Determines whether an account is locked out based on the lockoutTime attribute.
    /// </summary>
    /// <param name="entry">The search result entry containing lockoutTime.</param>
    /// <returns>True if the account is locked out; otherwise, false.</returns>
    /// <remarks>
    /// An account is considered locked if lockoutTime is present and not zero.
    /// </remarks>
    ...
    /// <summary>
    /// Gets the list of group distinguished names that the user is a member of.
    /// </summary>
    /// <param name="entry">The search result entry containing the memberOf attribute.</param>
    /// <returns>A list of group distinguished names, or an empty list if none.</returns>
    ...
    /// <summary>
    /// Parses an Active Directory generalized time string to a DateTimeOffset.
    /// </summary>
    /// <param name="value">The generalized time string (format: yyyyMMddHHmmss.0Z).</param>
    ...
    /// <summary>
    /// Parses a Windows FILETIME value to a DateTimeOffset.
    /// </summary>
    /// <param name="value">The FILETIME value as a string (100-nanosecond intervals since January 1, 1601).</param>
    /// <returns>The parsed DateTimeOffset, or null if parsing fails or value is zero.</returns>
    ...
    /// <summary>
    /// Gets the base distinguished name for LDAP searches.
    /// </summary>
    /// <returns>The configured base DN, or a generated one from the domain name.</returns>
    /// <remarks>
    /// If BaseDn is configured, returns it directly. Otherwise, generates a base DN
    /// from the domain name (e.g., "contoso.com" becomes "DC=contoso,DC=com").
    /// </remarks>
    ...
    /// <remarks>
    /// Returns standard Active Directory user attributes including identity,
    /// contact information, organizational data, and account status fields.
    /// </remarks>
